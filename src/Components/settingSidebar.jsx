...
function Sidebar({ isOpen, closeSidebar }) {
  ...
  const inBtnRef = useRef(null);
  const outerLayer = useRef(null);
  ...
  useEffect(() => {
    gsap.to(inBtnRef.current, {
      x: isOn ? 0 : 20,
      backgroundColor: isOn ? "#e5e7ab" : "#e5e7ab",
      duration: 0.4,
      ease: "elastic.out(1, 0.5)",
    });
    gsap.to(outerLayer.current, {
      backgroundColor: isOn ? "#3b3b3b" : "#40e353"
    })
  }, [isOn]);
  ...
  const menuItems = [
    ...
    },
    {
      ...
      button: (<div className="flex items-center ml-10">
        <button
          ref={outerLayer}
          onClick={handleToggle}
          className="w-12 bg-green-500 px-1 py-1 rounded-2xl flex items-center cursor-pointer"
        >
          <div
            ref={inBtnRef}
            className="bg-zinc-200 w-5 h-5 rounded-2xl"
          ></div>
        </button>
      </div>)
    },
  ...
  return (
    <>
      <aside
        ref={sidebarRef}
        className="fixed top-0 z-55 right-0 w-80 h-screen bg-zinc-900/80 backdrop-blur-lg p-6 flex flex-col gap-6 transform translate-x-full z-50 shadow-[0_0_30px_rgba(0,0,0,0.6)] rounded-l-3xl border-l border-zinc-800"
      >
        {/* Close button */}
        <button
